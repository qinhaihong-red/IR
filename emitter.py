...
class Emitter(object):
    ...
    def parent_variable_name(self, IR_node, path_or_name=[0]):
        if isinstance(path_or_name, _string_types):
            path = [IR_node.in_nodes_names.index(path_or_name)]
        elif isinstance(path_or_name, list):
            path = path_or_name
        else:
            raise ValueError
        return self.IR_graph.get_parent_variable_name(IR_node.name, path)#IR_graph定义在子类中，如PytorchEmitter中
